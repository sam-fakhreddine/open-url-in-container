...
async function setSigningKey(key) {
    await browser.storage.local.set({ [SIGNING_KEY_NAME]: key })
}
...
export async function getSigningKey() {
    const existingKey = (await browser.storage.local.get(SIGNING_KEY_NAME))[
        SIGNING_KEY_NAME
    ]
    if (!existingKey) {
        ...
        try {
            ...
            console.error('Error setting signing key:', e) // eslint-disable-line no-console
        }
        return newKey
    }
    return existingKey
...
export async function restoreState(component, initialState = {}) {
    const state = await browser.storage.local.get(component)
    return state[component] || initialState
}

export async function saveState(component, state) {
    await browser.storage.local.set({ [component]: state })
}
